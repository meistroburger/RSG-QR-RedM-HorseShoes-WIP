...
function fitShoe(player, horse)
    ...
    if player.job.name ~= "blacksmith" then 
        player:sendChatMessage("You are not a blacksmith.")
        return
    end
...
-- Register an event for when a player purchases a horse
RegisterServerEvent("player:purchaseHorse")
AddEventHandler("player:purchaseHorse", function(horse)
    -- Add the horse to the horse_shoes table with 0 shoes
    db:query("INSERT INTO horse_shoes (horse_id, shoe_count) VALUES (?, 0)", {horse:getHorseId()})
end)

-- Register an event for when a player is riding a horse
RegisterServerEvent("player:ridingHorse")
AddEventHandler("player:ridingHorse", function(player, horse)
    calculateHorseShoeLossChance(player, horse)
    updateHorseSpeed(horse)
end)
